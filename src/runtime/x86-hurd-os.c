...
{
    asm ("fldcw %0" : : "m" (context->uc_mcontext.fpregs.fp_reg_set.fpchip_state.status));
}
...
{
    return &context->uc_mcontext.gregs[REG_UESP];
}
...
{
    switch(offset) {
    case reg_EAX: return &context->uc_mcontext.gregs[REG_EAX];
    case reg_ECX: return &context->uc_mcontext.gregs[REG_ECX];
    case reg_EDX: return &context->uc_mcontext.gregs[REG_EDX];
    case reg_EBX: return &context->uc_mcontext.gregs[REG_EBX];
    case reg_ESP: return &context->uc_mcontext.gregs[REG_ESP];
    case reg_EBP: return &context->uc_mcontext.gregs[REG_EBP];
    case reg_ESI: return &context->uc_mcontext.gregs[REG_ESI];
    case reg_EDI: return &context->uc_mcontext.gregs[REG_EDI];
    default: return 0;
    }
